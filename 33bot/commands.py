...
# define commands
async def golden_gun(client, message):
    """
...
async def roll_the_dice(client, message):
    await client.send_message(message.channel, "Nothing's Here. Any Suggestions?")


async def shaxx(client, message):
    # join the VC
    vc = None
    ...
    try:
        ...
        await client.send_message(message.channel, "LET ME FINISH!")
